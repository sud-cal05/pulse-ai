"""
Zero-shot probe (DESIGN.md §10 process, §21 Phase 4).
...
This is the concrete version of the process described in DESIGN.md §10:
"run zero-shot first on your golden set, record the mistakes, then add
a few-shot example targeting each mistake class." The few-shot file in
this repo was already built anticipating these failure modes — running
this script is how you VERIFY that anticipation was correct, and catch
anything it missed (which becomes a new example in v2).
...
# Deliberately the hard cases — this is where zero-shot vs few-shot
# should actually diverge. Plain, easy items would look identical either
# way and wouldn't tell you anything useful.
PROBE_ITEMS = [
...
def run_probe() -> None:
    ...
    print(
        ...
        "(DESIGN.md §9 prompt versioning) rather than editing v1 in place."
    )
